...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"door\":\"%d\"", threshold_value_init.Distance_value); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
...
unsigned char Mqtt_Pwd_Info(char *buf)
{
	...
	strcpy(buf, "{");
		...
		memset(text, 0, sizeof(text));
		sprintf(text, "\"set\":\"%d\"", device_state_init.DoorPwd); // Temp是数据流的一个名称，temper是温度值
		strcat(buf, text);
